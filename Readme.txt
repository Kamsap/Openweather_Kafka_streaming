...
PROJET : STREAMING AVEC KAFKA

*********************************************
Nom des membres :
-	Florian FOUALEM
-	Ronel NJOGANG
-	Abiola David DJAKOPO
-	Ines Armelle MATCHEU
-	Junior KAMSAP
-       zahra Haj SALEM 

*********************************************
Introduction:
Création d’un Pipeline de collecte automatisé en temps réel des données météorologiques de la France à partir d’une API open data.
*********************************************

Étape 1 : Recherche et Sélection d'API
+ Choix de l'API : on a choisi d'utiliser OpenWeatherMap, qui est une API bien établie pour les données météorologiques. Pour ce projet, nous utiliserons l'API gratuite "Current Weather Data" d'OpenWeatherMap, qui fournit des informations météorologiques en temps réel.
+ Accéder à l'API : on s'inscrie sur OpenWeatherMap et on obtient une clé API (API key).
+ On télécharge le fichier "city.list.json.gz", et on extrait son contenu pour obtenir la liste des villes françaises  (le script python import_data.py nous montre qu’on a 19965 villes francaise identifiées).
...
On crèe un fichier qu'on nomme "producer.py" ;  à l'intérieur duquel on écrit le script du producer. Pour tester ce producteur, on aura besoin d’un cluster Kafka en cours d’exécution localement 
...
+ Écrire les Dockerfiles pour chaque service : dashboard, consumer, et producer
+ On écrit le fichier docker-compose.yml

******************************************************************************
Étape 10 à 11 : Test, Optimisation et Documentation
Lancez l'ensemble de l'infrastructure avec Docker Compose : *******sudo docker-compose up***********
...
On accède au tableau de bord à l'adresse  « http://localhost : 8501 »
